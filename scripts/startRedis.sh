...
docker run -d --name="$name-cluster" -p "$startPort-$endPort:$startPort-$endPort" jamespedwards42/alpine-redis-testing:unstable "$startPort" "$numNodes" --protected-mode no --save \"\" --repl-diskless-sync yes --cluster-enabled yes --appendfsync no --activerehashing no --cluster-announce-ip "$announceIp"
