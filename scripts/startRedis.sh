...
docker run -d --name="$name-cluster" -p "$startPort-$endPort:$startPort-$endPort" jamespedwards42/alpine-redis-testing:unstable redis-server "$startPort" "$numNodes" --cluster-enabled yes --cluster-announce-ip "$announceIp"
